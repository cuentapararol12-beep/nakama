/* ═══════════════════════════════════════════════════════════════
   ONE PIECE RPG — SHARED ENGINE  v3.2
   Cargado por: index.html · free-mode.html · level-mode.html
   CAMBIOS v3.2:
   - ENE y ESP usan Math.floor en tramos de nivel (confirmado NNM staff)
   - Sin emojis en ningun mensaje ni render
   - editMode en estado ('npc' | 'character')
═══════════════════════════════════════════════════════════════ */

const EMBEDDED = {
  "version":"3.2",
  "rules":{"baseStatMain":3,"baseStatPower":0,"freePointsPerLevel":46,"autoGainPerLevelMain":3},
  "mainAttributes":["FUE","RES","AGI","REF","INT","VOL","DES","PRE"],
  ...
  "attributesMeta":{
    ...
    "DES":{"label":"Destreza","icon":""},"PRE":{"label":"Precision","icon":""},
    "CH": {"label":"Control Haki","icon":""},  "CA":{"label":"Control Akuma","icon":""}
  },
  "thresholds":[
...
function mkState() {
  return {
    ...
    levelCtx   : 'bulk',
    karmas     : {},
    belico: { weapons: [], techniques: [] },  // { karmaId: value } — value for fixed karmas, or chosen int for variable
  };
}
...
/* ── Derived stats ──────────────────────────────────────────── */
// Formulas oficiales NNM (confirmadas por staff):
//   ENE: floor(L/10)*500 + floor(L/25)*1000  (tramos completos, no division continua)
//   ESP: floor(L/15)*250
...
function patchSave(d) {
  ...
  // migración: weapon singular → array
  ...
  (d.belico.techniques||[]).forEach(t => {
    ...
    if (t.isDual   === undefined) t.isDual   = false;
...
function renderTimeline() {
  ...
  for (let l=S.startLevel+1;l<=S.level;l++) {
    ...
    ch.textContent=l; ch.title=`Nivel ${l}: ${lvlSpent(l)}/46 pts`;
...
function renderLvlSummary() {
  ...
  if(ls) ls.innerHTML=`
    ...
    <div class="lsi"><div class="lsi-l">Libre total Nv${S.level}</div><div class="lsi-v">${S.level*46} pts</div></div>
...
/* ═══════════════════════════════════════════════════════════════
   BÉLICO — parse de armas, técnicas y cálculo de daño
═══════════════════════════════════════════════════════════════ */
const _BSTAT = {
...
function bStat(n) {
  ...
  return _BSTAT[norm] || n.toUpperCase();
}
...
function parseWeaponBlock(raw) {
  ...
  scalStr.split('/').forEach(part => {
    ...
    if (m) scaling[bStat(m[2])] = parseFloat(m[1].replace(',','.'));
  });
...
function calcWeaponDmgBreakdown(weapon, statsOv, opts={}) {
  ...
  Object.entries(weapon.scaling||{}).forEach(([s, baseM]) => {
    ...
    parts.push({ label:`${s}×${mult}${opts.dualPenalty?' (sin Ambi)':''}`, value:contrib, detail:`${statVal}×${mult}` });
    total += contrib;
  });
...
function parseTechniqueBlock(raw) {
  ...
  // Busca línea que sea solo un número (el nivel)
  let levelIdx = -1;
  ...
  // Fallback inline (todo en una línea)
  if (levelIdx===-1) {
...
function calcTechniqueDmgBreakdown(tech, weapons) {
  const isDual      = tech.isDual  || false;
  const hasAmbi     = tech.hasAmbi || false;
  const dualPenalty = isDual && !hasAmbi;
...
function checkReqs(reqStr) {
  return reqStr.split(/[|/]/).map(p => {
    ...
    if (!m) return { text: p, met: null };
    const val = parseInt(m[1]), stat = bStat(m[2]);
    if (!ALL.includes(stat)) return { text: p, met: null };
    return { text: p, met: effStat(stat) >= val, need: val, have: effStat(stat) };
  }).filter(Boolean);
}